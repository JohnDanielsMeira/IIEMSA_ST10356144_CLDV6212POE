...
namespace ABCRetailersST10356144.Services
{
    public class AzureStorageService : IAzureStorageService
    {
        ...
        private async Task InitializeStorageAsync()
        {
            ...
            { 
                _logger.LogError(e, "Failed to initialize Azure Storage: {Message}", e.Message);
        ...
        public async Task DeleteEntityAsync<T>(string partiotionKey, string rowKey) where T : class, ITableEntity, new()
        {
            var tableName = GetTableName<T>();
            var tableClient = _tableServiceClient.GetTableClient(tableName);

            await tableClient.DeleteEntityAsync(partiotionKey, rowKey);
        }
        ...
        public async Task<string> UploadImageAsync(IFormFile file, string containerName)
        {
            ...
            {
                _logger.LogError(ex, "Error uploading image to container {ConatinerName}: {Message}", containerName, ex.Message);
                throw;
            }
        }
        ...
        public async Task<string> UploadFileAsync(IFormFile file, string containerName)
        {
            ...
            {
                _logger.LogError(ex, "Error uploading file to container {ConatinerName}: {Message}", containerName, ex.Message);
                throw;
            }
        }
